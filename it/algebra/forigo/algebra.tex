...
\usepackage{amssymb}   % for nexists, varnothing
...
\section{Operatori e Quantificatori Logici}

$P \land Q$: $P$ e $Q$ \\
$P \lor Q$: $P$ o $Q$ \\
$\forall v \in D \ldotp P(v)$: Per ogni $v$ in $D$ vale $P(v)$ \\
$\exists v \in D : P(v)$: Esiste almeno un $v$ in $D$ tale che $P(v)$ \\
$\exists! v \in D : P(v)$: Esiste un unico $v$ in $D$ tale che $P(v)$



\section{Definizione di Insieme}

Un \textbf{insieme} (set) è una collezione di oggetti non ordinata e senza molteplicità (senza ripetizioni). \\
...
\subsection{Modalità di Definizione di un Insieme}

Un insieme può essere definito: \\

- per \textbf{elencazione}: scrivendo gli elementi tra parentesi graffe, separati da virgole. \\
...
- per \textbf{proprietà caratteristica} o per comprensione (set comprehension / set-builder notation): indicando una proprietà P che caratterizza gli elementi dell'insieme X \\

\indent\indent
$\forall o \in Y \ldotp o \in X \subseteq Y \iff P(o)$ \\
...
La \textbf{cardinalità} (cardinality) $\card A = n \in \N$ di un insieme $A$ è il numero di elementi in $A$.



\section{Definizione di Sottoinsieme}

Un insieme $X$ si dice \textbf{sottoinsieme} (subset) di un insieme Y, scritto $X \subseteq Y$, se ogni elemento di $X$ è anche un elemento di $Y$. \\

$X \subseteq Y \iff \forall e \in X \ldotp e \in Y$



\section{Definizione di Sottoinsieme Proprio}

Un insieme $X$ si dice \textbf{sottoinsieme proprio} (proper subset) di un insieme Y, scritto $X \subset Y$, se $X$ è diverso da $Y$ e ogni elemento di $X$ è anche un elemento di $Y$. \\

$X \subset Y \iff X \neq Y \land \forall e \in X \ldotp e \in Y$



\section{Unione di Insiemi}

L'\textbf{unione} (union) $A \cup B$ di due insiemi A e B è l'insieme contenente sia gli elementi di A che di B. \\

$A \cup B = \{ e : e \in A \lor e \in B \}$.



\section{Intersezione di Insiemi}

L'\textbf{intersezione} (intersection) $A \cap B$ di due insiemi A e B è l'insieme contentente gli elementi che sono sia elementi di A che elementi di B. \\

$A \cap B = \{ e : e \in A \land e \in B \}$.



\section{Differenza di Insiemi}

La \textbf{differenza} (difference) $A - B$ o $A \setminus B$ di due insiemi A e B è l'insieme contentente solo gli elementi di A che non sono elementi di B. \\

$A - B = A \setminus B = \{ e : e \in A \lor e \notin B \}$.



\section{Multiinsiemi o Sistemi}

Un \textbf{multiinsieme} (multiset) o sistema è una collezione non ordinata di elementi con molteplicità (ovvero un insieme con ripetizioni). \\
...
Il \textbf{prodotto cartesiano} (cartesian product) $A \times B$ di due insiemi $A$ e $B$ è l'insieme contenente tutte le coppie di due elementi dove il primo elemento appartiene ad A ed il secondo elemento appartiene a B. \\
...
Si dice che $R \subseteq A \times B$ è una relazione (binaria, anche detta corrispondenza) ((binary) relation) tra due insiemi $A$ e $B$.\\
...
\noindent
$S$ è detta \textbf{libera} se l'unica combinazione lineare dei suoi  elementi che da $\underline 0$ è quella a coefficienti tutti nulli. I vettori di $S$ si dicono \textbf{linearmente indipendenti} (linearly independent). \\

\noindent
$S$ è detta \textbf{legata} se esiste almeno una combinazione lineare dei suoi elementi a coefficienti non tutti nulli che da {\underline 0}. I vettori di $S$ si dicono \textbf{linearmente dipendenti} (linearly dependent). \\
...
\noindent
La somma $U + W$ si dice \textbf{diretta} (direct sum) e si scrive $U \oplus W$ se ogni elemento di $U + W$ si scrive in modo unico come somma di un elemento di $U$ ed un elemento di $W$. \\
...
$B_U \cup B_W$ è una base di $U \oplus W$ \\
...
\noindent
$\cof_{i,j}(A) = c_{i,j} = (-1)^{i+j} \detm{A_{i,j}}$ \\

\noindent
La trasposta della matrice dei cofattori di A è detta \textbf{matrice aggiunta} $\adj A$ o $A_a$ o $A^a$ (adjugate matrix / classical adjoint of a square matrix / adjunct matrix / ``adjoint'') \\

\noindent
$\adj A = (\cof A)^T$ \\

\noindent
$\detm A \neq 0 \implies A^{-1} = \frac{1}{\detm A} \cdot \adj A$
...
\noindent
Osservazione: \\
I valori di una trasformazione lineare $f : V(\K) \rightarrow W(\K)$ dipendono solamente dai valori di $f$ sui vettori di una base di $V(\K)$, perché: \\
