...
Un campo $(\K, +, \cdot)$ è un anello commutativo unitario in cui $(\K^*, \cdot)$ è un gruppo abelliano, dove $\K^* := \K \setminus \braces{0}$. \\
...
% TODO Dimostrazione della legge di annullamento del prodotto
...
\indent
$\forall a, b \in \K, \textbf{v} \in V \ldotp (ab)\textbf{v} = a(b\textbf{v})$ \\

\noindent
$*$ ammette elemento neutro (\textbf{unitarietà}) \\
...
\noindent
Curiosità: \\
Le ultime quattro proprietà dicono che il prodotto per scalare definisce un omomorfismo tra l'anello del campo $\K$ ($(\K, +)$) e l'anello degli endomorfismi del gruppo $(V, \oplus)$.
