...
% Per dimostrare la seconda, cioè che un oggetto ($\mathcal L(X)$) è il più piccolo che soddisfa una certa proprietà, mostriamo che: $Y \leq V(\K) (è sottospazio) e $X \subseteq Y$ \implies \mathcal L \subseteq Y$
...
% $S$ legata $\implies \mathcal L(S) = \mathcal L(S \setminus \braces {\overbar v_i$ dove $\overbar v_i$ è un vettore $\in S$ che è combinazione lineare di altri vettori in $S$.
...
\section{Metodo di Completamento della Base}
...
Una funzione $f : V(\K) \rightarrow W(\K)$ è detta \textbf{trasformazione}/\textbf{applicazione}/mappa lineare (o omomorphismo di spazi vettoriali) (linear transformation/map o vector space homomorphism) se associa combinazioni lineari in uno spazio vettoriale $V(\K)$ a combinazioni lineari in un altro spazio vettoriale $W(\K)$. \\
...
\noindent
Una trasformazione lineare è invertibile $\iff$ è un isomorfismo (lineare) ((linear) isomorphism)
